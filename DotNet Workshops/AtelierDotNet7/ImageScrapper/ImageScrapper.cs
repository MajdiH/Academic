                        ...
                        using (StreamReader rdr = new StreamReader(s))
                        {
                            ...
                            Task[] tasks = new Task[] {
                                Task.Factory.StartNew(() => {
                                    Parallel.ForEach(this.regxImg.Matches(page).Cast<Match>(), img => DownloadImg(root, img.Groups[1].Value));
                                }),
                                Task.Factory.StartNew(() => {
                                    Parallel.ForEach(this.regxLink.Matches(page).Cast<Match>(), link =>
                                    {
                                        string sLink = link.Groups[1].Value;
                                        Uri url;
                            ...
                            Task.WaitAll(tasks); // attente de terminaison des tâches
